...
def search_tree():
    ...
    cursor.execute(sql_query, (date_today, f"%{query}%", f"%{query}%"))
...
def root_okay(period):
    global root88, search_var, p
    p = period
    root88 = Tk()
    ...
    heading = Label(root88, text="List of Present Student in current Period", fg="white", background="#326273", font=("Times New Roman", 29))
    heading.place(x=370, y=30)
...
def tree():
    ...
    initial_query = """
        ...
        FROM attendance a 
        JOIN student s ON a.uid = s.uid 
        WHERE a.date = %s and a.period = %s
    """
    cursor.execute(initial_query, (date_today,p))
